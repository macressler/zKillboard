...
class Points
{
	...
	public static function setPoints(&$json)
	{
		$zkb = $json["zkb"];
		$vicpoints = isset($json["victim"]["shipType"]["group"]) ? self::getPoints($json["victim"]["shipType"]["group"]) : 1;
		$vicpoints += $zkb["totalValue"] / 10000000;
		$maxpoints = round($vicpoints * 1.2);

		$invpoints = 0;
		foreach ($json["attackers"] as $inv)
		{
			$invpoints += isset($inv["shipType"]["group"]) ? self::getPoints($inv["shipType"]["group"]) : 1;
		}
		...
		$zkb["points"] = (int) max(1, round($points, 0)); // a kill is always worth at least one point
		$json["zkb"] = $zkb;
	}
}
